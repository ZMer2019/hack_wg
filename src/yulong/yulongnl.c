...
int netlink_redirect_addr_request(uint32_t sid, pid_t comm_pid){
    ...
    head = genlmsg_put(skb, comm_pid, 0, &genl_family, 0, WG_CMD_REDIRECT_ADDR_REQUEST);
    ...
    do{
        err = nla_put_u32(skb, WGACL_A_UUID, sid);
        if(err != 0){
            break;
        }
        err = nla_put_s32(skb, WGACL_A_SEQ, request_seq);
    ...
    LOGI("netlink redirect addr request, comm_pid[%d]\n", comm_pid);
    login_wait(request_seq);
    return 0;
}